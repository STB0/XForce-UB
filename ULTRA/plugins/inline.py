...
import asyncio
import os
from XForce-UB import BOT, PHOTO, VERSION, MSG
import requests
import time
...
async def repo(event):
    if event.fwd_from:
        return
    XForce-UB = Var.TG_BOT_USER_NAME_BF_HER
    if event.reply_to_msg_id:
        await event.get_reply_message()
    response = await bot.inline_query(XForce-UB, "alive")
